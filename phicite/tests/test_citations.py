...
def test_create_citation_invalid_json(test_app_with_db):
    ...
    response = test_app_with_db.post("/citations/", json={})
...
def test_update_citation(test_app_with_db):
    ...
    response = test_app_with_db.put(f"/citations/{citation_id}/", json=update_payload)
    assert response.status_code == 200
...
def test_update_citation_invalid(test_app_with_db, id, payload, status_code, detail):
    response = test_app_with_db.put(f"/citations/{id}/", json=payload)
